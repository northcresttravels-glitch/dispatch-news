...
const SECTION_QUERIES = {
  ...
  finance: 'finance markets stocks',
...
app.get('/api/scrape', async (req, res) => {
  ...
  try {
    ...
    for (const p of pMatches) {
      const text = p.replace(/<[^>]+>/g, '').replace(/&[a-z]+;/gi, ' ').trim();
      if (text.length > 80) paragraphs.push(`<p>${text}</p>`);
    }

    if (paragraphs.length < 2) {
      return res.json({
        body: '<p>Full article not available. Please visit the original source.</p>',
        scraped: false
      });
    }

    return res.json({ body: paragraphs.slice(0, 15).join('\n'), scraped: true });

  } catch (err) {
    console.error('Scrape error:', err);
    return res.json({
      body: '<p>Could not load full article. Please visit the original source.</p>',
      scraped: false
    });
  }
});
...
app.get('/api/news', async (req, res) => {
  ...
  try {
    const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&sortBy=publishedAt&pageSize=12&language=en&apiKey=${process.env.NEWS_API_KEY}`;
    const response = await fetch(url);
    ...
    const stories = data.articles
      ...
      .map(a => ({
        headline: a.title.replace(/\s*-\s*[^-]*$/, '').trim(),
        deck: a.description || '',
...
app.listen(port, () => console.log(`The Pro Drafter running on port ${port}`));
